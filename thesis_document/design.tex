...
To create a latency trace the attack carefully configures a timer such that each instruction of the program is interrupted, effectively single-stepping through the program. 
The attacker than implements the Interrupt Service Routine (ISR) such that it logs the latency of the interrupted instruction \cite{nemesisrepo, sancussupport}. 
Any difference in latencies of corresponding instruction will be reflected in the latency trace and can leak information about secret-dependent branches. 

If the architecture has completely deterministic instruction latencies then a single latency trace is sufficient for leaking information about secret-dependent branches. 
...
For architectures such as the MSP430 the resulting latency traces will be indistinguishable. Architectures with more complex features will still have minor differences 
in the resulting traces due to random nature of the instruction latencies. 
However the variances will be smaller and an attacker will not be able to use statistical analysis to distinguish different instruction latencies. 
...
Two additional nodes are added as ancestor to the branch's leaf in order to increase the branch depth to three. The result of this process is shown in \ref{fig:equalize-branches-2}.
\begin{figure*}[t!]
    \centering
    \begin{subfigure}[t]{0.5\textwidth}
        \centering
