...
Finally section \ref{sec:rewriting} discusses the use of binary rewriting in the algorithm, the altering of a compiled program without access to its source code. 
...
In their paper Pouyanrad et al. \cite{MSP430Detection} have formally defined the Nemesis-Sensitive property.
...
The first stage of the algorithm therefore consists of inserting additional nodes into the CFG, such that all path lengths to a given node are equal as well as all branch depths. 
...
\begin{figure*}[t!]
    ...
    \begin{subfigure}[t]{0.5\textwidth}
        ...
        \label{fig:unequal}
    \end{subfigure}
    \caption{Problematic structures in CFG alignment procedure}
...
The immediate dominator is determined for each node reachable from $n$. 
If all leaves reachable from $n$ have the same immediate dominator $d$, then all paths from $n$ to some leaf go through $d$. 
...
If such a node $d$ exists, then the nodes that are a part of the sub-graph are all nodes that are on a path from $n$ to $d$. 
...
Removing the cycle is done by removing the edge from the CFG that connects the deepest node to the most shallow node. 
...
An index variable $i_{ref}$ is used to keep track of the position of the first instruction that has not yet been aligned. 
...
\label{sec:nop}
Any instruction that is inserted into the program cannot have an effect on its outcome. In this regard they are the same as the no-operation instruction and are 
...
RetroWrite can be expanded in a similar way, extending support of the algorithm to both ARM and x86\_64 devices. 
