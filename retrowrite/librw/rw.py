...
class Symbolizer():
    ...
    def symbolize_text_section(self, container, context):
        ...
        for rel in container.relocations[".text"]:
            fn = container.function_of_address(rel['offset'])  # get the function in which relocation is contained
            if not fn or fn.name in Rewriter.GCC_FUNCTIONS:
                continue

            inst = fn.instruction_of_address(rel['offset'])  # get instruction inside this function in which relocation is contained
            if not inst:
                continue
    ...
    def symbolize_cf_transfer(self, container, context=None):
        # symbolize control flow transfer
        for _, function in container.functions.items():
            # create a dictionary mapping instruction addresses to id's (counting up from 0)
            addr_to_idx = dict()
            ...
            for inst_idx, instruction in enumerate(function.cache):
                ...
                # populate functions.nexts[inst_idx] -- keep track of which instruction (identifief using inst_idx) follows current inst_idx
                # the instruction is not a call nor a jump, so simple control flow
                ...
                # the instruction is a jump, but not a call (I guess a call is a subset of a jump)
                if is_jmp and not instruction.mnemonic.startswith("jmp"):
                    if inst_idx + 1 < len(function.cache):
                        # Add natural flow edge --
                        # (zelf) - the next instruction in case the jmp is not taken, I guess? so one possible next instruction is the inst with idx+1 ???
                        function.nexts[inst_idx].append(inst_idx + 1)
                    else:
                        # Out of function bounds, no idea what to do!
                        function.nexts[inst_idx].append("undef")
                elif is_call:  # the instruction is a call
                    instruction.cf_leaves_fn = True
                    function.nexts[inst_idx].append("call")
                    if inst_idx + 1 < len(function.cache):
                        # add next instruction to 'nexts' for current instruction (maybe because it will be the return address?)
                        function.nexts[inst_idx].append(inst_idx + 1)
                    else:
                        # Out of function bounds, no idea what to do!
                        function.nexts[inst_idx].append("undef")

                # instruction operand is an immediate value (?)
                if instruction.cs.operands[0].type == CS_OP_IMM:
                    ...
                    # if instruction is a jump to an immediate address, add the idx of that addres to 'nexts'
                    if is_jmp:
                    ...
                    # instruction is a jump but the operand is not an immediate value
                    function.nexts[inst_idx].append("undef")
